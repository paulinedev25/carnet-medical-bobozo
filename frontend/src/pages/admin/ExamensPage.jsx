...
export default function ExamensPage() {
  ...
  const normalizeStatut = (s) => {
    ...
    if (clean.toLowerCase().includes("encours") || clean.toLowerCase().includes("en_cours")) return "en_cours";
    if (clean.toLowerCase().startsWith("prescrit")) return "prescrit";
    return clean.toLowerCase();
  };

  // 🔹 Charger examens depuis API
  ...
  // 🔹 Gestion modales / CRUD
  const handleModalSave = async (payload) => {
    setSaving(true);
    try {
      if (examenToEdit && examenToEdit.id) {
        await updateExamen(examenToEdit.id, payload);
      ...
      console.error("Erreur création/modification examen:", err);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("⚠️ Voulez-vous vraiment supprimer cet examen ?")) return;
    setSaving(true);
    try {
  ...
  const handleSaveResult = async (payload) => {
    ...
    setSaving(true);
    try {
      await updateResultat(selected.id, payload);
      toast.success("✅ Résultats enregistrés");
      setOpenResultModal(false);
      setSelected(null);
      await loadExamens();
    } catch (err) {
      console.error("Erreur maj résultat:", err);
      toast.error(err?.response?.data?.error || "❌ Échec enregistrement résultats");
    } finally {
      setSaving(false);
    }
  };
  ...
  // 🔹 Téléchargement PDF côté serveur
  const handleDownloadPDF = async (exam) => {
    try {
      if (!exam?.id) return;
      const blob = await downloadExamenPDF(exam.id);
      const url = window.URL.createObjectURL(new Blob([blob], { type: "application/pdf" }));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `examen-${exam.id}.pdf`);
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded shadow">
        <div className="px-4 py-2 text-sm text-gray-600">
          {loading
            ? "Chargement..."
            : `Total ${filteredRows.length} examen${filteredRows.length > 1 ? "s" : ""}`}
        </div>
            ...
            <tbody>
              {filteredRows.map((e, idx) => (
                <tr key={e.id} className="border-t hover:bg-gray-50">
                      ...
                      <>
                        ...
                        <div>
                          👤 {e.consultation.patient
                            ? [e.consultation.patient.nom, e.consultation.patient.postnom, e.consultation.patient.prenom]
                                .filter(Boolean)
                                .join(" ")
                  ...
                  <td className="px-4 py-2">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${getStatutClass(e.statut)}`}>
                  ...
                  <td className="px-4 py-2 flex gap-2">
                    ...
                    {(user?.role === "medecin" || user?.role === "admin") && (
                      <button onClick={() => { setExamenToEdit(e); setOpenExamenModal(true); }} disabled={saving} className="px-2 py-1 rounded border hover:bg-yellow-100" title="Modifier la prescription">✏️</button>
                    )}
                    {(user?.role === "laborantin" || user?.role === "admin") && (
                      <button onClick={() => { setSelected(e); setOpenResultModal(true); }} className="px-2 py-1 rounded border hover:bg-gray-100" title="Saisir/Modifier résultats">🧪</button>
                    )}
                    {user?.role === "medecin" && normalizeStatut(e.statut) === "en_cours" && (
                      <button onClick={() => handleInterpret(e.id)} className="px-2 py-1 rounded border hover:bg-green-100" title="Interpréter">✅</button>
                    )}
                    {(user?.role === "admin" || user?.role === "medecin") && (
                      <button disabled={saving} onClick={() => handleDelete(e.id)} className="px-2 py-1 rounded border hover:bg-red-100" title="Supprimer">🗑️</button>
                    )}
              ...
              {!loading && filteredRows.length === 0 && (
                <tr>
                  <td colSpan={columns.length} className="text-center py-6 text-gray-500">Aucun examen trouvé</td>
                </tr>
              )}
            </tbody>
          </table>
